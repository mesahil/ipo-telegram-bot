                ...
                if is_not_found:
                    ...
                    if hasattr(client, 'find_fuzzy_matches'):
                        try:
                            ...
                            if fuzzy_matches:
                                logger.info(f"[POLL FUZZY] Found {len(fuzzy_matches)} potential fuzzy matches for '{ipo_name}'.")
                                keyboard_inline = []
                                for m in fuzzy_matches[:3]:
                                    ...
                                    keyboard_inline.append([{
                                        ...
                                        "callback_data": f"sub_allot:{registrar}:{m.match.replace(':',' ')}"
                                    }])
                                ...
                                msg_text = (
                                    ...
                                    f"Select a match below to check status immediately:"
                                )
                                payload = {
                                    "chat_id": chat_id,
                                    "text": msg_text,
